    ...
    // ============ 第 1 课：认识围棋 ============
    {
      ...
      steps: [
        { type: 'text', content: '围棋的棋盘由横竖交错的线组成，棋子下在线的交叉点上。9 路棋盘共有 81 个交叉点。本教程会带你一步步从零学会下围棋！' },
        {
          ...
          text: '怎么报位置？先看棋盘边上的坐标：<b>底部字母</b>是横坐标（列），从左到右为 A、B、C…J、K（跳过 I 避免与 1 混淆）；<b>左侧数字</b>是纵坐标（行），从下到上为 1～9。<br>每个交叉点都写作“<b>字母＋数字</b>”，例如正中央的“<b>天元</b>”就是 <b>E5</b>（第 E 列、第 5 行，画面上已标出），四角附近的“<b>星位</b>”分别是 C3、G3、C7、G7。<br>中文里也习惯按“<b>第几路</b>”来叫，把某点说成“<b>X 之 Y</b>”：比如“<b>三三</b>点”（第 3 路 × 第 3 路）、“<b>三五</b>点”（第 3 路 × 第 5 路，又叫“目外”）、“<b>四四</b>”（星位点）。'
        },
        {
          type: 'quiz',
          setup: '',
    ...
    // ============ 第 15 课：真眼与假眼 ============
    {
      ...
      steps: [
        ...
        },
        {
          ...
          text: '<b>假眼</b>：看画面，中间这个空点（绿色标记处）看起来也被黑棋围住了，但下面那颗黑子 <b>E4</b> 是<b>单独的、没和整块黑棋连上</b>。<br>白棋可以吃掉它，假眼立刻就会破掉——所以它只是<b>“假眼”</b>。做活时必须把假眼补成真眼。'
        },
        {
          type: 'quiz',
          setup: 'B 4,4 4,5 4,6 5,4 5,6 6,5',
          ...
          options: ['下面那颗黑子 E4 是单独的，没连上，会被吃掉', '因为白棋不能下进去', '因为黑棋围得太多了'],
          answer: 0,
          explanation: '中间空点下面那颗黑子 E4 与整块黑棋没连上，白棋能吃掉它，这个眼就会破——所以是假眼。'
        },
        {
          type: 'move',
          playerColor: GO.BLACK,
          setup: 'B 4,4 4,5 4,6 5,4 5,6 6,4 6,6',
          objective: '现在轮到你执黑。这颗“假眼”下面缺一块。请下在 <b>E4</b>，把下面连上，把假眼<b>补成真眼</b>。',
          hint: '下在 E4，让整块黑棋连成一个整体。',
          check: ({ board }) => {
            const done = board.eyePoints(GO.BLACK).includes(40);
            return {
              done,
              hint: done ? null : '请下在 E4，把下面那颗“断点”连上。',
      ...
      steps: [
        {
          ...
          frames: [
            ...
            { board: 'B 3,3 3,7 W 7,7', text: '黑棋沿上边<b>拆边</b>到 G7，扩大地盘。', mark: 24, duration: 1700 },
            { board: 'B 3,3 3,7 W 7,7 7,3', text: '白棋也沿下边<b>拆边</b>到 C3。布局完成：双方各占了角、拓了边。', mark: 56, duration: 2200 }
          ]
        },
        {
          type: 'quiz',
        ...
        },
        {
          ...
          objective: '收官阶段：这块白棋被困在角里、只剩一口气。请下在 <b>B7</b>，把它提掉（收官提子）。',
          hint: '白棋 B8 只剩最后一口气 B7，下在那里就能提掉它。',
          check: ({ board, lastMove }) => {
      ...
      steps: [
        {
          type: 'demo',
          frames: [
            { board: 'B 1,2 2,1 2,2 3,1 3,2 W 4,1 4,2', text: '看画面：黑棋在角上围出一个<b>“眼”</b>（空点 A9，绿圈）。白棋没有眼。<br>这个眼是黑棋的“永久气”——白棋<b>永远紧不掉它</b>。', highlights: [{ i: 0, label: '眼' }], duration: 2400 },
      ...
      steps: [
        {
          ...
          text: '看画面：白棋 D6 被黑棋从左右两侧 <b>“夹”</b>住，只剩上下两个出口。<br>黑棋下一步再紧一个方向，白棋就只剩一口气了。'
        },
        {
          type: 'move',
          playerColor: GO.BLACK,
          setup: 'B 3,4 5,4 W 4,4',
          objective: '现在轮到你执黑。白棋 D6 已被两侧夹住，请下在 <b>E6</b>，从上方夹紧它——白棋只剩一口气（打吃）。',
          hint: '白棋上下两个出口 C6 和 E6，下在 E6 夹紧。',
          check: ({ board, lastMove }) => {
            const done = lastMove === 31 && board.liberties(30).length === 1;
            return {
              done,
              hint: done ? null : '请下在 E6，夹紧白棋上方。',
      ...
      steps: [
        {
          type: 'demo',
          frames: [
            { board: 'B 3,3', text: '黑棋先在左上角星位 <b>C7</b> 占角。', mark: 20, duration: 1500 },
            { board: 'B 3,3 W 4,4', text: '白棋在 <b>D6</b> 点三三——直接点进黑棋的角里，抢角地！', mark: 30, duration: 1800 },
            { board: 'B 3,3 3,4 W 4,4', text: '黑棋在 <b>D7</b> 挡住白棋，不让它往这边出头。', mark: 21, duration: 1600 },
            { board: 'B 3,3 3,4 W 4,4 4,3', text: '白棋再往 <b>C6</b> 爬一手。', mark: 29, duration: 1500 },
            { board: 'B 3,3 3,4 3,5 W 4,4 4,3', text: '黑棋在 <b>E7</b> 再挡。', mark: 22, duration: 1600 },
        ...
        },
        {
          ...
          playerColor: GO.BLACK,
          setup: 'B 3,3 W 4,4',
          objective: '现在轮到你执黑。白棋点三三 D6 抢角，请按定式在 <b>D7</b> 挡住白棋。',
          hint: '下在 D7，挡住白棋往左上发展。',
          check: ({ board, lastMove }) => {
            const done = lastMove === 21;
            return {
              done,
              hint: done ? null : '请下在 D7 挡。',
