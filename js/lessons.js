...
(function (global) {
  ...
  const LESSONS = [
      ...
      steps: [
        {
          ...
          frames: [
            ...
            { board: 'B 1,2 2,1 3,2 2,3 1,9 W 1,3 3,3 2,4 2,9', text: '白棋这一手<b>能提掉</b>黑子（第 4 课学过“能提子就能下”）——但这里有个<b>例外中的例外</b>！黑棋刚提走一颗白子，白棋只能在原位 B8 立刻提回这<b>同一颗子</b>，会<b>无限循环</b>。<br>所以<b>打劫规则</b>：<b>不能立刻在原位提回</b>，只能先去<b>别处找劫材</b>——于是下在 J8，打吃右上角黑子（绿圈是它仅剩的一口气）。', mark: 17, highlightLibs: 8, duration: 2800 },
        ...
        },
        {
          ...
          setup: 'B 1,2 2,1 3,2 2,3 W 1,3 3,3 2,4',
          ko: 10,
          objective: '现在轮到你执白。请先试试下在 B8 原位提回——虽然“能提子”，但程序会告诉你这是“打劫”，不允许立刻下。然后请下在别处（找劫材）。',
          hint: '试试直接点 B8，看程序的提示；然后选一个别的位置落子（找劫材）。',
          check: ({ lastMove }) => {
    ...
    // ============ 第 14 课：扑（倒扑） ============
    {
      id: 14,
      title: '吃子技巧④ · 扑（倒扑）',
      intro: '“扑”（也叫倒扑）不是直接吃子，而是故意送一子给对方提：对方一提，整块棋反而只剩一口气；你再回提，便能用一子换吃一大块。关键顺序是<b>扑 → 对方提 → 回提</b>。',
      steps: [
        {
          type: 'demo',
          frames: [
        ...
        },
        {
          ...
          options: ['G9——扑进连接点送一子，白棋提子后接不归，黑棋回提整块 7 子', 'G8——直接收气，但只能吃 6 子', '随便下，都只能吃 3 子'],
          answer: 0,
        ...
        {
          type: 'text',
          content: '倒扑只记一件事：<b>对方提掉你的扑子后，自己会不会只剩一口气？</b><br>若答案是“会”，再确认你回提时能吃掉对方整块，扑就成立。回提点看似像禁入点也不用怕：<b>能提掉对方就能下</b>。<br><b>和打劫的区别</b>：打劫是回提同一颗单子，可能无限循环；倒扑是回提吃掉对方整块，一次性结束。'
        }
      ]
    },
    // ============ 第 15 课：真眼与假眼 ============
    {
      id: 15,
      title: '死活基础① · 真眼与假眼',
      intro: '“眼”也有真假。判断方法不是只看空点四周有没有己方棋，而是要问：<b>对手能不能在这里落子，并吃掉一颗支撑这只眼的棋？</b>能，就是假眼；不能，才是真眼。',
      steps: [
        {
          type: 'visual',
        ...
        },
        {
          ...
          options: ['白棋能下在 E5，提掉只剩一口气的黑子 E4', '因为白棋不能下进去', '因为黑棋围得太多了'],
          answer: 0,
        ...
        {
          type: 'text',
          content: '<b>真眼检查口诀</b>：先看空点，再看围住它的每一颗棋。只要其中有一颗被打吃、对手能落在眼位把它提掉，这只眼就是假眼。做活时，先补断点、先救支撑子，再谈做眼。'
        }
      ]
    },
    // ============ 第 16 课：大眼死活 ============
      ...
      steps: [
        {
          ...
          text: '<b>直四（活棋）</b>：黑棋围住四个连成一线的空点（绿色标记处）。白棋先占其中一点后，剩下的空间仍能分成两处，黑棋能保留两只眼。<br>所以直四<b>天生就是活棋</b>，不用再补。'
        },
        {
          type: 'visual',
          ...
          text: '<b>刀五（需要补）</b>：黑棋围出五个空点，形状像一把“刀”。中间的 <b>F6</b> 是急所：黑棋先下这里，能把空间分成两眼；白棋先下这里，黑棋的眼位就被压成一团。<br>所以刀五不是“看到五个空点就活”，而是<b>必须先抢急所</b>。'
        },
        {
          type: 'demo',
          frames: [
        ...
        },
        {
          ...
          options: ['黑棋先下 F6，能把大眼分成两只眼', '因为 F6 离棋盘中央最近', '五个空点本来就一定活，不需要下'],
          answer: 0,
      ...
      steps: [
        {
          type: 'visual',
          setup: 'B 3,3 3,7',
          text: '看画面：黑棋先占左上角星位 C7，又沿上边<b>“拆边”</b>到 G7——两个子隔着 3 个点，既不容易被切断，又占住了上边一大片地盘。<br>布局时，<b>占角 → 守角 → 拆边</b>，是扩充地盘的基本套路。'
        },
        {
          type: 'quiz',
          setup: 'B 3,3',
          question: '黑棋占了左上角星位 C7。为了沿上边发展、扩充地盘，黑棋下一步最好下在哪里？',
          options: ['G7：沿边拆边，占住上边地盘', 'E5：下到中央', 'A9：紧贴角落'],
          answer: 0,
          explanation: '占角之后要“拆边”，沿边扩张。G7 与 C7 隔着 3 个点，占住了上边一大片，是最常见的拆边。'
        },
        {
          type: 'move',
          playerColor: GO.BLACK,
      ...
      steps: [
        {
          ...
          highlights: [{ i: 22 }],
          text: '看画面：黑棋有两块棋被白棋隔开，中间 <b>E7</b>（绿色标记处）是个“断点”。<br>如果白棋先下在 E7，黑棋就被彻底切断；黑棋先连上，就变成一块厚棋。<b>抢先连接自己的断点</b>，是中盘的重要思路。'
        },
        {
          type: 'quiz',
          setup: 'B 3,3 3,4 3,6 3,7 W 4,4 4,6',
          ...
          options: ['先下在 E7 把两块连起来', '下到别处去，不管它', '主动把断点让给白棋'],
          answer: 0,
        ...
        },
        {
          ...
          playerColor: GO.BLACK,
          setup: 'B 3,3 3,4 3,6 3,7 W 4,4 4,6',
          objective: '现在轮到你执黑。请下在 <b>E7</b>，把两块黑棋连接起来。',
          hint: '两块黑棋之间的断点就是 E7。',
          check: ({ board, lastMove }) => {
            if (lastMove === null) return { done: false };
    ...
    // ============ 第 20 课：收官与全局思路 ============
    {
      id: 20,
      title: '收官与全局思路',
      intro: '一盘棋不只是“从头下到尾”。先看局面属于哪个阶段，再做该阶段最重要的事：布局抢落脚点，中盘处理强弱，收官把已经得到的优势兑现成目数。',
      steps: [
        {
          type: 'text',
          content: '<b>先认阶段，再找重点。</b><br><b>布局</b>：双方棋子离得远，主要争角和边，目标是建立根据地。<br><b>中盘</b>：黑白开始接触，出现断点、弱棋和打吃，目标是先处理棋的死活与强弱。<br><b>收官</b>：大块棋大致安定，只剩边界和小空点，目标是围牢、补净、数清每一目。'
        },
        {
          type: 'demo',
          frames: [
            { board: 'B 3,3 W 7,7', text: '<b>布局：</b>双方棋子还没有接触。黑棋先占左上角 C7，白棋占右下角 G3；这时最值钱的是角和边。', mark: 20, duration: 2000 },
            { board: 'B 3,3 3,7 W 7,7', text: '<b>布局：</b>黑棋再占右上角 C3，先给自己搭好根据地。布局阶段棋子离得远，主要争角和边。', mark: 24, duration: 2000 },
            { board: 'B 3,3 3,7 W 7,7 7,3', text: '<b>布局：</b>白棋占到左下角 G7。双方各占两角、互不接触——这就是布局：先让每块棋都有落脚点，别急着在中央打架。', mark: 56, duration: 2000 }
          ]
        },
        {
          type: 'demo',
          frames: [
            { board: 'B 3,3 3,4 3,6 3,7 W 4,4 4,6', text: '<b>中盘：</b>黑白开始接触。黑棋中间的 E7 是断点，局面的重点不再是“多占一个角”，而是先别被切断。', highlights: [{ i: 22, label: '断点' }], duration: 2400 },
            { board: 'B 3,3 3,4 3,5 3,6 3,7 W 4,4 4,6', text: '<b>中盘：</b>黑棋补在 E7，先把两块棋连成一块。处理弱棋、连接断点，往往比贪图小地更重要。', mark: 22, duration: 2200 }
          ]
        },
        {
          type: 'demo',
          frames: [
            { board: 'B 1,2 2,1 2,3 W 2,2', text: '<b>收官：</b>大局已基本安定，只剩角上的一颗白棋还没处理。它只剩 B7 一口气；现在每一手都在兑现已经围住的地。', highlightLibs: 10, duration: 2400 },
            { board: 'B 1,2 2,1 2,3 3,2', text: '<b>收官：</b>黑棋下 B7 提掉白子，角部边界彻底收净。收官不是重新开战，而是把能确定的目数稳稳拿到手。', mark: 19, duration: 2200 }
          ]
        },
        {
          type: 'quiz',
          setup: 'B 3,3 3,4 3,6 3,7 W 4,4 4,6',
          highlights: [{ i: 22, label: '断点' }],
          question: '看画面：黑白棋已经接触，黑棋还有一个断点。现在最像哪个阶段？首先该处理什么？',
          options: ['中盘：先连接断点、处理弱棋', '布局：先去远处占另一个角', '收官：马上数地'],
          answer: 0,
          explanation: '棋子接触后，局面进入中盘。黑棋若不先连断点，就会被白棋切成两块弱棋；这时处理强弱比抢小地更急。'
        },
        {
          type: 'move',
          playerColor: GO.BLACK,
          setup: 'B 3,3 3,4 3,6 3,7 W 4,4 4,6',
          objective: '中盘练习：黑棋的两块棋被白棋隔开。请先下在 <b>E7</b>，连接断点、让黑棋安定下来。',
          hint: '中盘先问“哪块棋最弱”。两块黑棋之间的 E7 就是必须先处理的断点。',
          check: ({ board, lastMove }) => {
            const done = lastMove === 22 && board.groupOf(22).length >= 5;
            return {
              done,
              hint: done ? null : '请下在 E7，把两块黑棋连接起来。',
              successMsg: done ? '很好！中盘先把断点连上，黑棋就不再是两块弱棋。记住：战斗中先处理棋的强弱，再考虑小目数。' : null
            };
          }
        },
        {
          ...
          objective: '收官练习：白棋 B8 已经没有发展空间，只剩 B7 一口气。请下在 <b>B7</b> 提掉它，把角部边界收净。',
          hint: '收官时先看“还能确定拿到什么”。白棋唯一的气在 B7，下在那里就能提掉它。',
          check: ({ board, lastMove }) => {
            const done = lastMove === 19 && board.grid[10] === GO.EMPTY;
            return {
              done,
              hint: done ? null : '白棋 B8 只剩一口气，下在 B7 提掉它。',
              successMsg: done ? '很好！你把已经确定能拿到的角部收干净了。收官的感觉就是：少冒险、多兑现，最后再数地判断胜负。' : null
            };
          }
        },
        {
          type: 'text',
          content: '<b>全局判断口诀：</b><br>棋子离得远，先占角边做根据地；<br>黑白开始接触，先看气、断点和弱棋；<br>大块棋安定后，再把边界围牢、把小目数收净。<br>每次落子前先问自己：<b>我现在在哪个阶段？这阶段最急的事是什么？</b>'
        }
      ]
    },
    // ============ 第 21 课：虎口与关门吃 ============
      ...
      steps: [
        {
          ...
          text: '看画面：黑棋 D7、E7 和白棋 D6、E6 上下紧贴，谁也逃不掉——这就是一对<b>“对杀”</b>。<br>要判断胜负，先数气：每块棋的<b>空邻居</b>（不是己方也不是对方棋子的点）都是它的气。'
        },
        {
          type: 'quiz',
          setup: 'B 3,4 3,5 W 4,4 4,5',
          ...
          answer: 0,
          explanation: '黑棋 D7、E7 的气：上面 D8、E8、左边 C7、右边 F7 共 4 口气（下面 D6、E6 是白棋，不算）。白棋的气：下面 D5、E5、左边 C6、右边 F6 共 4 口气。双方同气 4。'
        },
        {
          type: 'quiz',
          setup: 'B 3,4 3,5 W 4,4 4,5',
          question: '现在是同气对杀（黑、白各 4 气）。黑棋先手，想紧白棋的气、抢占先机，该下在哪里？',
          options: ['C6：堵住白棋左边的气', 'C7：这是黑棋自己的气，不紧白棋', '白棋气太多，紧不完'],
          answer: 0,
          explanation: '白棋 D6、E6 的气在 C6、F6、D5、E5 四处。黑棋先手应抢先紧白棋的气（如 C6），让白棋的气变少——对杀中抢先紧气才有利，这正是下一课要学的“先走者胜”。'
        }
      ]
    },
    // ============ 第 29 课：对杀② 同气先走者胜 ============
      ...
      steps: [
        {
          type: 'visual',
          setup: 'B 3,4 3,5 W 4,4 4,5',
          text: '看画面：黑棋 D7、E7 和白棋 D6、E6 各 <b>4 口气</b>，是同气对杀。<br>现在轮到黑棋先走——黑棋抢先“紧白棋的气”（下在 C6），白棋的气就变少，黑棋抢到了先机。这就是<b>“同气先走者胜”</b>：双方气一样时，先动手紧气的一方占便宜。'
        },
        {
          type: 'move',
          playerColor: GO.BLACK,
          setup: 'B 3,4 3,5 W 4,4 4,5',
          objective: '现在轮到你执黑（先手）。请抢先紧白棋的气——下在 <b>C6</b>，把白棋的气从 4 口紧到 3 口。',
          hint: '白棋的气在 C6、F6、D5、E5，下在 C6 紧气。',
          check: ({ board, lastMove }) => {
            const done = lastMove === 29 && board.liberties(30).length === 3;
            return {
              done,
              hint: done ? null : '请下在 C6，紧白棋的气。',
    ...
    // ============ 第 30 课：对杀③ 有眼杀无眼 ============
    {
      id: 30,
      title: '对杀入门④ · 有眼杀无眼',
      intro: '真眼会让对方无法直接落子紧气。在本课这种<b>封闭、外气相近</b>的对杀中，有真眼的一方占据决定性优势；实战仍要一起比较外气、共气和手数。',
      steps: [
        {
          type: 'demo',
          frames: [
            { board: 'B 1,2 2,1 2,2 3,1 3,2 W 4,1 4,2', text: '看画面：黑棋在角上围出一个<b>“眼”</b>（空点 A9，绿圈）。白棋没有眼。<br>在这个封闭棋形里，这只眼让白棋不能直接紧黑棋的关键气。', highlights: [{ i: 0, label: '眼' }], duration: 2400 },
            { board: 'B 1,2 2,1 2,2 3,1 3,2 W 4,1 4,2', text: '为什么紧不掉？白棋如果想下在眼 A9 里紧气——看，它上下左右全是黑子，<b>这是禁入点</b>（红 ✕），白棋下进去就是“自杀”，规则不允许！', highlights: [{ i: 0, style: 'forbidden' }], duration: 2800 },
            { board: 'B 1,2 2,1 2,2 3,1 3,2 4,1 W 4,2', text: '黑棋可以继续紧白棋的气；白棋却不能直接填入黑棋的眼。<br>因此在这道<b>封闭对杀题</b>里，黑棋的关键气更难被紧掉，最终能先吃掉白棋。这就是“有眼杀无眼”在简单棋形中的用法。', mark: 27, duration: 2600 }
          ]
        },
        {
          type: 'quiz',
          ...
          options: ['黑棋有真眼，白棋不能直接填眼紧气，黑棋能先紧白棋', '黑棋的棋子更大', '没有原因，只是传说'],
          answer: 0,
          explanation: '白棋直接下入 A9 会自杀，不能马上填眼紧气。这个局部又没有额外外气差异，所以黑棋能先把白棋的气紧完。遇到更复杂的对杀，还要继续数外气、共气和手数。'
        }
      ]
    },
    // ============ 第 31 课：好形与愚形 ============
    {
      ...
      steps: [
        ...
        },
        {
          ...
          setup: 'B 4,4 4,5 5,4',
          highlights: [{ i: 40 }],
          text: '再看画面：黑棋三颗子围出一个<b>“空三角”</b>（愚形三角）——中间 E5 那个点（绿圈）自己留着没什么用，三颗子挤在一起、互相堵塞。<br>这种棋形又笨重、又容易被攻击，是<b>愚形</b>，要尽量避免。'
        },
        {
          type: 'quiz',
          setup: 'B 4,4 4,6',
    ...
    // ============ 第 32 课：手筋① 扳 ============
    {
      ...
      steps: [
        ...
        },
        {
          ...
          playerColor: GO.BLACK,
          setup: 'W 3,4 3,5 B 4,4 4,5',
          objective: '现在轮到你执黑。口诀“二子头必扳”——请下在 <b>D8</b>，扳住白棋二子头的左侧。',
      ...
      steps: [
        {
          ...
          text: '看画面：黑棋两块棋 D6 和 F4 被白棋隔开，中间的 <b>E5</b> 是联络的关键点。<br>黑棋下在 E5（斜对角，叫“尖”），就能把两块黑棋联络起来，白棋再难切断。'
        },
        {
          type: 'move',
          playerColor: GO.BLACK,
          setup: 'B 4,4 6,6 W 5,4 5,6',
          objective: '现在轮到你执黑。请用“尖”联络两块黑棋——下在 <b>E5</b>。',
          hint: '两块黑棋之间的斜对角 E5 就是“尖”的位置。',
          check: ({ board, lastMove }) => {
    ...
    // ============ 第 34 课：手筋③ 夹 ============
    {
      ...
      steps: [
        {
          type: 'visual',
          setup: 'B 4,3 4,5 W 4,4',
          text: '看画面：白棋 D6 被黑棋从左右两侧 <b>“夹”</b>住，只剩上下两个出口。<br>黑棋下一步再从上方紧一手，白棋就只剩一口气了。'
        },
        {
          type: 'move',
          playerColor: GO.BLACK,
          setup: 'B 4,3 4,5 W 4,4',
          objective: '现在轮到你执黑。白棋 D6 已被两侧夹住，请下在 <b>D7</b>，从上方夹紧它——白棋只剩一口气（打吃）。',
          hint: '白棋上下两个出口 D7 和 D5，下在 D7 夹紧。',
          check: ({ board, lastMove }) => {
            const done = lastMove === 21 && board.liberties(30).length === 1;
            return {
              done,
              hint: done ? null : '请下在 D7，夹紧白棋上方。',
      ...
      steps: [
        {
          ...
          text: '看画面：白棋两颗子 C7 和 E7 之间隔着一个空点 <b>D7</b>，它们想从这里连上。<br>黑棋抢先下在 D7，就能像楔子一样把白棋<b>挖断</b>，让白棋变成两块孤棋。'
        },
        {
          type: 'move',
          playerColor: GO.BLACK,
          setup: 'W 3,3 3,5 B 2,3 4,3',
          objective: '现在轮到你执黑。请下在 <b>D7</b>，挖进白棋的联络点，把白棋切成两半。',
          hint: '两块白棋之间的空点 D7 就是挖断的位置。',
          check: ({ board, lastMove }) => {
            const done = lastMove === 21;
            return {
              done,
              hint: done ? null : '请下在 D7，挖断白棋的联络。',
              successMsg: done ? '好！你“挖”断了白棋：白棋 C7 和 E7 被黑子隔开，变成两块孤棋，黑棋接下来可以分而攻之。' : null
            };
          }
        }
      ]
    ...
    // ============ 第 37 课：定式入门 · 小目一间挂 ============
    {
      id: 37,
      title: '定式入门① · 小目一间挂',
      intro: '“定式”是角上常见的局部应对。它不是必须背下的唯一答案；先理解双方在争什么，再通过实战熟悉常见手法。今天看“小目一间挂”。',
      steps: [
        {
          type: 'demo',
          frames: [
            { board: 'B 3,4', text: '黑棋先在小目 <b>D7</b> 占角。小目是 9 路常见的占角点。', mark: 21, duration: 1500 },
            { board: 'B 3,4 W 4,3', text: '白棋在 <b>C6</b> 挂角（隔着一个小目点，叫“一间挂”），准备进攻黑角。', mark: 29, duration: 1800 },
            { board: 'B 3,4 W 4,3 3,2', text: '白棋继续在 <b>B7</b> 托——贴着黑棋 D7 下方，试探黑棋的应手。', mark: 19, duration: 1800 },
            { board: 'B 3,4 4,4 W 4,3 3,2', text: '黑棋在 <b>D6</b> 扳住白棋。', mark: 30, duration: 1600 },
            { board: 'B 3,4 4,4 3,5 W 4,3 3,2', text: '黑棋再在 <b>E7</b> 长一手，巩固角部。', mark: 22, duration: 1600 },
            { board: 'B 3,4 4,4 3,5 W 4,3 3,2 4,2', text: '白棋也在 <b>B6</b> 立下站稳。<br>一个简单的<b>小目一间挂定式</b>就下完了——黑棋守住了角，白棋也在边上站稳了，双方都能接受。', mark: 28, duration: 2400 }
          ]
        },
        {
          type: 'move',
          ...
          objective: '现在轮到你执黑。白棋挂角又托，请按照定式，在 <b>D6</b> 扳住白棋，守住你的角。',
          hint: '下在 D6，扳住白棋 C6。',
          check: ({ board, lastMove }) => {
            const done = lastMove === 30;
            return {
              done,
              hint: done ? null : '请下在 D6 扳住白棋。',
              successMsg: done ? '好！你按定式在 D6 扳住白棋，黑棋守住了角部——这就是小目一间挂的基本应对。' : null
            };
          }
        }
      ]
    ...
    // ============ 第 38 课：角部定型 · 三三入侵 ============
    {
      id: 38,
      title: '角部定型② · 三三入侵',
      intro: '三三入侵的目的，是用角地交换对方的外势。本课只练习局部取舍：白棋想要角地，黑棋要在外面保持厚势，不要求死背固定变化。',
      steps: [
        {
          type: 'demo',
          frames: [
      ...
      steps: [
        {
          ...
          setup: 'B 2,2 2,3 2,4 2,5 3,2 3,5 4,2 4,3 4,4 4,5 W 6,5 6,6 6,7 6,8 7,5 7,8 8,5 8,6 8,7 8,8',
          highlights: [{ i: 20 }, { i: 21 }, { i: 59 }, { i: 60 }],
          text: '看画面：一局接近尾声。黑棋在左上围了一块，白棋在右下围了一块。<br>黑棋围住 <b>C7、D7</b> 两个空点（绿圈），白棋围住 <b>F3、G3</b> 两个空点（绿圈）。<br>判断形势：不能只看地盘，还要数棋子（黑左上 10 子 vs 白右下 10 子，目前相当），再算贴目。'
        },
        {
          type: 'quiz',
          setup: 'B 2,2 2,3 2,4 2,5 3,2 3,5 4,2 4,3 4,4 4,5 W 6,5 6,6 6,7 6,8 7,5 7,8 8,5 8,6 8,7 8,8',
      ...
      steps: [
        {
          type: 'demo',
          frames: [
            { board: 'B 2,2 2,3 2,4 3,2 3,4 W 6,5 6,6 6,7 7,5 7,7', text: '看画面：收官阶段。左上角黑棋 C7 是个小官子——黑棋扳一下，能多围住一个点。', duration: 2200 },
            { board: 'B 2,2 2,3 2,4 3,2 3,3 3,4 W 6,5 6,6 6,7 7,5 7,7', text: '黑棋在 <b>C7</b> 扳，多围住一个点。<br>黑棋这手算<b>“先手官子”</b>吗？——要看白棋要不要应。', mark: 20, duration: 2000 },
            { board: 'B 2,2 2,3 2,4 3,2 3,3 3,4 W 5,5 6,5 6,6 6,7 7,5 7,7', text: '白棋如果<b>不应</b>黑棋的扳，白棋就在 <b>E5</b> 拆一手、扩大自己的地盘——这样黑棋的扳就变成了“后手”，白棋占了便宜。<br><b>先手官子</b>：对方不敢不应、必须跟着应的官子，要抢着下；能抢到先手，就多赚一手棋。', mark: 40, duration: 3200 }
          ]
        },
        {
          type: 'quiz',
    ...
    // ============ 第 41 课：毕业测试 ============
    {
      ...
      steps: [
        {
          type: 'text',
          content: '恭喜你学完全部课程！最后是<b>毕业测试</b>：和电脑独立下完一整盘 9 路棋。<br>对局时试着用上你学到的：<b>围地为主、吃子为辅</b>；注意连接、小心断点；<b>被征子就别跑了</b>（电脑现在会征你）；收官记得<b>抢先手</b>。终局点击“数地”查看胜负——完成这一局，你就毕业了！'
        },
        {
          type: 'free',
          playerColor: GO.BLACK,
          objective: '🎓 毕业测试：执黑对战电脑，独立下完一整局，终局点击“数地”查看胜负。完成这一局，你就毕业了！'
        }
      ]
    }
  ];
