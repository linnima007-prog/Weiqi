...
(function (global) {
  ...
  const LESSONS = [
    ...
    // ============ 第 7 课：眼与活棋 ============
    {
      id: 7,
      title: '眼与活棋',
      intro: '先认识“做出两只牢靠的眼就能活”。眼也有真假；怎样分辨真假眼会在第 15 课学习。',
      steps: [
        {
          type: 'visual',
          ...
          text: '看画面：这块黑棋围出了<b>两只牢靠的眼</b>（绿色标记处）。白棋不能同时填掉两只眼，因此这块棋是<b>“活棋”</b>，不会被吃。<br>这里的眼都很牢靠；有些看似被围住的空点其实是假眼，第 15 课再专门辨别。'
        },
        {
          type: 'quiz',
          setup: 'B 2,3 2,4 2,5 2,6 2,7 3,3 3,5 3,6 3,8 4,3 4,4 4,5 4,6 4,7 4,8',
    ...
    // ============ 第 10 课：终局与数地 ============
    {
      id: 10,
      title: '终局与数地',
      intro: '对局结束后要用约定的计分方法判定胜负。本课先完成第一盘练习对局；后续学完更多战斗知识，再参加毕业实战。',
      steps: [
        {
          type: 'visual',
          setup: 'B 2,2 2,3 2,4 2,5 3,2 3,5 4,2 4,3 4,4 4,5 W 6,5 6,6 6,7 6,8 7,5 7,8 8,5 8,6 8,7 8,8',
          highlights: [{ i: 20 }, { i: 21 }, { i: 59 }, { i: 60 }],
          text: '当双方都觉得没有棋可下时（通常双方都连续“放弃一手”），对局就结束了。<br>本应用的是<b>简化面积计分</b>：看画面，黑棋在左上围了 <b>2 个空点</b>（绿圈），白棋在右下也围了 <b>2 个空点</b>（绿圈），将各自的<b>棋子数 + 围住的空点数</b>相加。黑棋先手有优势，因此白棋加 7.5 目贴目。不同比赛规则的数法会不同。'
        },
        {
          type: 'quiz',
          setup: 'B 2,2 2,3 2,4 2,5 3,2 3,5 4,2 4,3 4,4 4,5 W 6,5 6,6 6,7 6,8 7,5 7,8 8,5 8,6 8,7 8,8',
          highlights: [{ i: 20 }, { i: 21 }, { i: 59 }, { i: 60 }],
          question: '本应用的简化面积计分中，绿圈空点怎样参与胜负？',
          options: ['和己方棋子一起相加，比总数并结合贴目', '只看谁棋子下得多', '先动手的人赢'],
          answer: 0,
          explanation: '画面里绿圈标出的就是各自围住的空点。本应用“棋子 + 围住的空点”的简化面积计分；黑方的先手优势由白棋 7.5 目贴目补偿。'
        },
        {
          type: 'free',
          playerColor: GO.BLACK,
          objective: '第一盘练习对局：执黑对战电脑。尽量围地、照顾被打吃的棋；当你觉得下完了，点击“数地”查看本应用的简化面积计分结果。'
        }
      ]
    },
    // ============ 第 11 课：征子 ============
      ...
      steps: [
        {
          type: 'visual',
          setup: 'B 3,3 3,7',
          text: '看画面：在<b>没有白棋打入或接近</b>的教学局面，黑棋占左上角星位 C7 后，再沿上边<b>“拆边”</b>到 G7。两个子隔着 3 个点，既留有发展空间，也开始争上边。<br>这不是固定套路：实战要先看对手离多近、哪一边更宽，再决定守角、拆边还是接近对方。'
        },
        {
          type: 'quiz',
          setup: 'B 3,3',
          question: '在这道“上边没有白棋干扰”的练习中，黑棋想沿上边发展，哪个点是常见选择？',
          options: ['G7：沿边拆边，开始争上边', 'E5：因为所有局面都必须先下中央', 'A9：紧贴角落，且不看局面'],
          answer: 0,
          explanation: '在没有白棋干扰的本题中，G7 是常见的沿边发展点。实战还要根据对手位置调整方向和距离，不能机械照搬。'
        },
        {
          type: 'move',
          playerColor: GO.BLACK,
          ...
          check: ({ lastMove }) => ({
            ...
            successMsg: '很好！你在这道无干扰练习中沿边拆开了。下一步实战要继续观察对手位置，决定这块棋该守、该拆还是该接触。'
          })
        }
      ]
    },
    // ============ 第 19 课：中盘攻防 ============
    {
      ...
      steps: [
        ...
        },
        {
          ...
          setup: 'B 3,3 3,4 3,6 3,7 W 4,4 4,6',
          highlights: [{ i: 22 }],
          question: '这道局部题里，黑棋先该问什么，并优先处理哪里？',
          options: ['“哪块最弱？”——此处先下 E7 连住断点', '“哪里最远？”——完全不看断点', '主动把断点让给白棋'],
          answer: 0,
          explanation: '被切断的棋会变弱。本题没有更急的死活或打吃，因此黑棋先抢 E7；实战要先比较“死活、打吃、断点”谁最急。'
        },
        {
          type: 'move',
          playerColor: GO.BLACK,
    ...
    // ============ 第 20 课：收官与全局思路 ============
    {
      ...
      steps: [
        ...
        },
        {
          type: 'demo',
          frames: [
            { board: 'B 1,2 2,1 2,3 W 2,2', text: '<b>收束局部：</b>大局已基本安定，只剩角上的一颗白棋还没处理。它只剩 B7 一口气；此时先拿到已确定的利益。', highlightLibs: 10, duration: 2400 },
            { board: 'B 1,2 2,1 2,3 3,2', text: '<b>收束局部：</b>黑棋下 B7 提掉白子，角部边界彻底收净。这是“兑现已确定利益”的例子；真正官子的先后手和大小将在第 40 课专门比较。', mark: 19, duration: 2200 }
          ]
        },
        {
          type: 'quiz',
        ...
        },
        {
          ...
          objective: '收束练习：白棋 B8 只剩 B7 一口气。请下在 <b>B7</b> 提掉它，把角部边界收净。',
          ...
          check: ({ board, lastMove }) => {
            ...
            return {
              ...
              successMsg: done ? '很好！你把已经确定能拿到的角部收干净了。先后手官子和目数大小会在后面专门练习。' : null
            };
          }
        },
        {
    ...
    // ============ 第 28 课：对杀① 分辨对象与数气 ============
    {
      id: 28,
      title: '对杀入门② · 分辨对象与数气',
      intro: '对杀要先分辨：双方是否真的无法脱身或连接。本课先在一个“只比较图中外气”的局部题里练习数气；实战还必须把逃跑、接应和共气一起算进去。',
      steps: [
        {
          type: 'visual',
          setup: 'B 3,4 3,5 W 4,4 4,5',
          text: '把画面外当作已经封住、不再有接应的教学局部：黑棋 D7、E7 和白棋 D6、E6 上下紧贴，接下来只比较谁先紧完图中的外气。<br>要判断这种对杀题，先数气：每块棋的<b>空邻居</b>（不是己方也不是对方棋子的点）都是它的气。'
        },
        {
          type: 'quiz',
          setup: 'B 3,4 3,5 W 4,4 4,5',
        ...
        },
        {
          type: 'quiz',
          setup: 'B 3,4 3,5 W 4,4 4,5',
          question: '在“外围已封住”的同气局部（黑、白各 4 气）中，黑棋先手想紧白棋的气，该下在哪里？',
    ...
    // ============ 第 29 课：对杀② 同气先走者胜 ============
    {
      id: 29,
      title: '对杀入门③ · 同气对杀先走者胜',
      intro: '在双方外气、共气、眼和后续手数都相同的封闭对杀中，同气时先紧对方气的一方获胜。先确认条件，再比较先后手。',
      steps: [
        {
          type: 'visual',
          setup: 'B 3,4 3,5 W 4,4 4,5',
          text: '沿用上一课“外围已封住、没有共气和眼”的局部前提：黑棋 D7、E7 和白棋 D6、E6 各 <b>4 口气</b>。<br>现在轮到黑棋先走——黑棋抢先紧白棋的气（下在 C6），白棋的气变少，黑棋抢到先机。这才是<b>“同气先走者胜”</b>适用的条件。'
        },
        {
          type: 'move',
          playerColor: GO.BLACK,
          ...
          check: ({ board, lastMove }) => {
            ...
            return {
              ...
              successMsg: done ? '好！在本题的封闭前提下，你抢先紧白棋的气、抢到先机。实战还要先核对外气、共气、眼和接应。' : null
            };
          }
        }
      ]
      ...
      steps: [
        {
          type: 'demo',
          frames: [
            { board: 'B 1,2 2,1 2,2 3,1 3,2 W 4,1 4,2', text: '把图外的气和接应都视为已经封住：黑棋在角上围出一个<b>真眼</b>（空点 A9，绿圈），白棋没有眼。这个局部中，黑棋多了一个白棋无法填入的内部空间。', highlights: [{ i: 0, label: '真眼' }], duration: 2400 },
            ...
            { board: 'B 1,2 2,1 2,2 3,1 3,2 4,1 W 4,2', text: '黑棋继续紧白棋的气；白棋却不能直接填入黑棋的眼。<br>因此在这道<b>假定外围已封住</b>的对杀题里，黑棋有决定性优势。实战不能只说“有眼就赢”，仍要数外气、共气和手数。', mark: 27, duration: 2600 }
          ]
        },
        {
          type: 'quiz',
    ...
    // ============ 第 31 课：好形与愚形 ============
    {
      id: 31,
      title: '棋形① · 好形与愚形',
      intro: '棋形要服务于局面。跳、飞、长常让棋形舒展；空三角常效率低，但遇到补断、做眼或吃子时也可能必要。',
      steps: [
        {
          type: 'visual',
          setup: 'B 4,4 4,6',
          text: '看画面：在周围没有紧急战斗时，黑棋两颗子隔着一个空点 <b>“跳”</b>——棋形开阔，进可攻退可守，是常见的好形。<br>跳过的中间点还留着，将来可以再决定是否连接。'
        },
        {
          type: 'visual',
          setup: 'B 4,4 4,5 5,4',
          highlights: [{ i: 40 }],
          text: '再看画面：在此处没有补断或吃子需要时，黑棋三颗子围出一个<b>“空三角”</b>（愚形三角）——中间 E5 那个点（绿圈）暂时用不上，三颗子挤在一起、效率偏低。<br>通常应避免无目的的愚形；若它能救棋、做眼或吃子，则另当别论。'
        },
        {
          type: 'quiz',
          setup: 'B 4,4 4,6',
          ...
          answer: 0,
          explanation: '本题没有紧急战斗，跳、飞等舒展棋形更高效；空三角通常效率低。判断棋形时仍要回到局面：它有没有救棋、补断或吃子的作用？'
        }
      ]
    },
    // ============ 第 32 课：手筋① 扳 ============
    {
      id: 32,
      title: '手筋① · 扳（二子头必扳）',
      intro: '“扳”是贴着对方棋子转向的一手。口诀“二子头要扳”提醒你留意对方的头部；是否真要扳，仍要先看己方断点和对方反击。',
      steps: [
        {
          type: 'visual',
        ...
        },
        {
          ...
          check: ({ board, lastMove }) => {
            ...
            return {
              ...
              successMsg: done ? '好！本题里你扳住了白棋二子头、限制它向上发展。记口诀是为了发现候选点，实战仍要比较其他急所。' : null
            };
          }
        }
      ]
    ...
    // ============ 第 33 课：手筋② 尖 ============
    {
      id: 33,
      title: '手筋② · 尖（联络的手筋）',
      intro: '“尖”是在己方棋子的斜对角下一手，常用于联络或补形。它让棋形紧凑，但是否安全仍要计算对方有没有切断手段。',
      steps: [
        {
          type: 'visual',
          setup: 'B 4,4 6,6 W 5,4 5,6',
          text: '看画面：黑棋两块棋 D6 和 F4 被白棋隔开，中间的 <b>E5</b> 是本题的联络关键点。<br>黑棋下在 E5（斜对角，叫“尖”）能形成紧凑联络；实战还要继续算白棋是否有切断或反击。'
        },
        {
          type: 'move',
          playerColor: GO.BLACK,
          ...
          check: ({ board, lastMove }) => {
            ...
            return {
              ...
              successMsg: done ? '好！你下了一手“尖”，完成本题的联络。尖常是联络、防守的候选手，但不能不看局面地套用。' : null
            };
          }
        }
      ]
    ...
    // ============ 第 34 课：手筋③ 夹 ============
    {
      id: 34,
      title: '手筋③ · 夹（两侧夹攻）',
      intro: '“夹”是从两侧限制对方的发展。本课的局部图里，夹后还能顺势紧气；实战中对方也可能反击或借力逃出。',
      steps: [
        {
          type: 'visual',
        ...
        },
        {
          ...
          check: ({ board, lastMove }) => {
            ...
            return {
              ...
              successMsg: done ? '好！本题中你从上方紧气，白棋只剩一口气。夹的目的通常是限制、攻击或制造薄味，不是看到两侧就必然能吃。' : null
            };
          }
        }
      ]
    ...
    // ============ 第 35 课：切断的手筋 · 挖 ============
    {
      id: 35,
      title: '切断的手筋 · 挖',
      intro: '“挖”是深入对方联络薄弱处的切断手筋。本课先练习最简单的断点；能否吃到棋，还要看双方气和周围接应。',
      steps: [
        {
          type: 'visual',
        ...
        },
        {
          ...
          check: ({ board, lastMove }) => {
            ...
            return {
              ...
              successMsg: done ? '好！你占住断点，白棋 C7 和 E7 被隔开。接下来是否能攻击或吃到其中一块，要继续数气、看接应。' : null
            };
          }
        }
      ]
    ...
    // ============ 第 37 课：定式入门 · 小目一间挂 ============
    {
      id: 37,
      title: '选修角部变化① · 小目挂角',
      intro: '角部变化没有必须背下的唯一答案。本课只看一条合法的示例顺序：先理解双方在争角地还是外势，再通过实战熟悉。',
      steps: [
        {
          type: 'demo',
          frames: [
            { board: 'B 3,4', text: '黑棋先在小目 <b>D7</b> 占角。小目是 9 路常见的占角点。', mark: 21, duration: 1500 },
            { board: 'B 3,4 W 4,3', text: '白棋在 <b>C6</b> 挂角，准备接近黑角。黑棋和白棋接下来都要选择：黑棋偏守角还是取外势，白棋偏取角地还是向边发展。', mark: 29, duration: 1800 },
            { board: 'B 3,4 4,4 W 4,3', text: '这里示范黑棋先在 <b>D6</b> 扳住白棋，限制白棋的方向。注意：这是黑棋的应手，轮到黑棋落子。', mark: 30, duration: 1600 },
            { board: 'B 3,4 4,4 W 4,3 3,2', text: '白棋在 <b>B7</b> 托，继续向角里寻求安定。', mark: 19, duration: 1800 },
            { board: 'B 3,4 4,4 3,5 W 4,3 3,2', text: '黑棋在 <b>E7</b> 长一手，巩固角部并保留外侧。', mark: 22, duration: 1600 },
            { board: 'B 3,4 4,4 3,5 W 4,3 3,2 4,2', text: '白棋在 <b>B6</b> 立下站稳。<br>这是一条<b>示例变化</b>：黑棋偏角部和外侧，白棋在边上安定。不要死背；换一处对方棋子，合理应手也会不同。', mark: 28, duration: 2400 }
          ]
        },
        {
          type: 'move',
          playerColor: GO.BLACK,
          setup: 'B 3,4 W 4,3',
          objective: '示例练习：白棋挂角后，黑棋选择在 <b>D6</b> 扳住它，限制白棋的方向。',
          hint: '下在 D6，扳住白棋 C6。',
          check: ({ board, lastMove }) => {
            ...
            successMsg: done ? '好！你走出了本例的应手。记住这里的目标是限制方向、争取角部，而不是把 D6 当作所有局面唯一答案。' : null
            };
          }
        }
      ]
    ...
    // ============ 第 39 课：形势判断基础 ============
    {
      id: 39,
      title: '形势判断基础 · 谁优谁劣',
      intro: '形势判断先分清已确定与未确定的部分。本应用以“棋子 + 围住的空点”的简化面积计分作粗算，再结合贴目决定该攻还是该守。',
      steps: [
        {
          type: 'visual',
          setup: 'B 2,2 2,3 2,4 2,5 3,2 3,5 4,2 4,3 4,4 4,5 W 6,5 6,6 6,7 6,8 7,5 7,8 8,5 8,6 8,7 8,8',
          highlights: [{ i: 20 }, { i: 21 }, { i: 59 }, { i: 60 }],
          text: '看画面：一局接近尾声，双方的空已确定。黑棋在左上围住 <b>C7、D7</b> 两个空点（绿圈），白棋在右下围住 <b>F3、G3</b> 两个空点（绿圈）。<br>按本应用的简化面积计分，还要数棋子：黑左上 10 子、白右下 10 子，目前相当；最后再计白棋 7.5 目贴目。实战的未定死活和劫争不能这样直接硬数。'
        },
        {
          type: 'quiz',
          setup: 'B 2,2 2,3 2,4 2,5 3,2 3,5 4,2 4,3 4,4 4,5 W 6,5 6,6 6,7 6,8 7,5 7,8 8,5 8,6 8,7 8,8',
    ...
    // ============ 第 40 课：官子入门 · 先手与后手 ============
    {
      id: 40,
      title: '官子入门 · 先手与后手',
      intro: '“官子”是收官阶段争夺小目的着法。判断先后手，不是看自己下后赚了几目，而是看对方不应会不会立刻吃亏。',
      steps: [
        {
          type: 'demo',
          frames: [
            { board: 'B 4,5 5,4 W 5,5', text: '先用一个最短局部看“先手”：白棋 E5 还有两口气。黑棋若在 F5 紧一手，白棋会只剩一口气。', duration: 2200 },
            { board: 'B 4,5 5,4 5,6 W 5,5', text: '黑棋在 <b>F5</b> 打吃。此时白棋若不应，黑棋下一手就会提子；白棋因此必须回应。', mark: 41, duration: 2000 },
            { board: 'B 4,5 5,4 5,6 W 5,5 6,5', text: '白棋只好在 <b>E4</b> 逃出。黑棋这手逼白棋回应后，下一手仍由黑棋在别处选择——这就是<b>先手</b>。<br>收官时也用同一问题判断：对方若不应，是否会立刻亏？若会，才是先手官子；大小还要另算。', mark: 49, duration: 3200 }
          ]
        },
        {
          type: 'quiz',
          setup: 'B 4,5 5,4 5,6 W 5,5',
          question: '黑棋打吃后，为什么说这手在局部是“先手”？',
          options: ['白棋不应就会被提，因此必须回应；官子同样这样判断', '白棋可以不理，去下任意大场', '只要自己多围一目就一定是先手'],
          answer: 0,
          explanation: '先手的关键是“对方不应会立刻吃亏”，而不是自己这一手看起来赚了多少。先手官子要和大小一起比较：通常先抢对方必须应、且价值大的官子。'
        },
        {
          type: 'free',
          playerColor: GO.BLACK,
          objective: '综合实战：执黑对战电脑。收束阶段先问“对方不应会立刻亏吗？”；若是，再比较这一手和其他官子的大小。终局点击“数地”查看本应用的简化面积计分结果。'
        }
      ]
    },
    // ============ 第 41 课：毕业测试 ============
